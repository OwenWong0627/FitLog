...
func (a *App) Workouts(c *fiber.Ctx, dbConn *sql.DB) error {
	...
	if tokenString == "" {
		return c.Status(http.StatusBadRequest).JSON(fiber.Map{"success": false, "errors": []string{"Token empty"}})
	}

	token, err := a.ParseJWT(tokenString)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(fiber.Map{"success": false, "errors": []string{"Token fetch failed"}})
	}
	...
	if !ok {
		return c.Status(http.StatusUnauthorized).JSON(&fiber.Map{"success": true, "error": []string{"Username DNE"}})
	}
	user := &db.User{}
...
func (a *App) Login(c *fiber.Ctx, dbConn *sql.DB) error {
	...
	if err := dbConn.QueryRow(db.GetUserByUsernameQuery, u.Username).
		...
		if err == sql.ErrNoRows {
			...
			getUserResp, dbErr := a.CognitoClient.GetUser(getUser)
			_, err = dbConn.Query(db.CreateUserQuery, u.Username, getUserResp.UserAttributes[len(getUserResp.UserAttributes)-1].Value)
			if dbErr != nil {
				return c.Status(http.StatusInternalServerError).JSON(fiber.Map{"success": false, "errors": []string{err.Error()}})
			}
		}
